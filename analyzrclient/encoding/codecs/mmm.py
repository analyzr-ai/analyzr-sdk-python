"""Codec for marketing mix model (MMM) result decoding.

Decodes training statistics, contribution data, lag analysis, media spend,
and response curve frames returned by the analytics engine after an MMM run.
Also provides a static helper for decoding optimizer output parameters.
"""
...
class MMMCodec(DomainCodec):
    ...
    def get_train_frame_names(self, config: Any) -> list[str]:
        """Return buffer frame names required to decode MMM train results.

        :param config: MMM training configuration (unused; included for interface parity).
        :return: List of the ten MMM output frame names.
        :rtype: list[str]
        """
        return [
            "train_stats",
    ...
    def decode_train_results(
        ...
        **kwargs: Any,
        ...
        """Decode MMM buffer frames into an ``MMMTrainResult``.

        Applies column-name decoding via fref_exp to all data and time-series frames
        when encoding is active.  Numeric type coercions are applied unconditionally.

        :param frames: Buffer frames containing the ten MMM output DataFrames.
        :param keys: Encoding keys; uses ``fref_exp`` for column-name decoding.
        :param config: MMM training configuration.
        :param model_id: Unique identifier for the trained model.
        :param encoding: Decode column names when ``True``.
        :return: Decoded MMM train result.
        ...
        if encoding and not train_data.empty:
            train_data.columns = FieldNameEncoder.decode_columns(
                train_data.columns, fref
            )
        ...
        if encoding and not contrib_stats.empty:
            contrib_stats.columns = FieldNameEncoder.decode_columns(
                contrib_stats.columns, fref
            )
        ...
        if encoding and not media_spend.empty:
            media_spend.columns = FieldNameEncoder.decode_columns(
                media_spend.columns, fref
            )

        contrib_data = frames.get("contrib_data", pd.DataFrame())
        if encoding and not contrib_data.empty:
            contrib_data.columns = FieldNameEncoder.decode_columns(
                contrib_data.columns, fref
            )

        resp_curve_media = frames.get("resp_curve_media", pd.DataFrame())
        if encoding and not resp_curve_media.empty:
            resp_curve_media.columns = FieldNameEncoder.decode_columns(
                resp_curve_media.columns, fref
            )

        resp_curve_pred = frames.get("resp_curve_pred", pd.DataFrame())
        if encoding and not resp_curve_pred.empty:
            resp_curve_pred.columns = FieldNameEncoder.decode_columns(
                resp_curve_pred.columns, fref
            )
        ...
        return MMMTrainResult(
            ...
            resp_curve_media=resp_curve_media,
            resp_curve_pred=resp_curve_pred,
        )
    ...
    def decode_optimize_results(
        ...
        if encoding:
            for i, val in data["Parameter"].items():
                if str(val)[:6] == "media_":
                    data.loc[i, "Parameter"] = (
                        f"media_{FieldNameEncoder.decode_value(str(val)[6:], fref)}"
                    )
        return data
