"""Runner for the regression pipeline (train and predict)."""
...
class RegressionRunner(BaseRunner):
    """Runs the regression pipeline (train and predict).

    Supports configurable algorithms, saturation/carryover variables,
    and hyperparameter grid search via ``param_grid``.
    ...
    def train(
        ...
        """Train a regression model and optionally poll for evaluation results.

        :param df: Input training DataFrame.
        :param config: Training configuration (algorithm, variable lists, train split, param grid).
        ...
        :return: Training result containing the model ID and evaluation metrics.
        ...
        if res["batches_saved"] == res["total_batches"]:
            self._client.post(
                self._uri,
                {
                    ...
                    "saturation_fields": [
                        fref["forward"][v] for v in config.saturation_vars
                    ]
                    if encoding
                    else config.saturation_vars,
                    "carryover_fields": [
                        fref["forward"][v] for v in config.lagging_vars
                    ]
                    if encoding
                    else config.lagging_vars,
                    "staging": staging,
    ...
    def predict(
        self,
        df: pd.DataFrame,
        config: RegressionPredictConfig,
        ...
        """Generate predictions from a pre-trained regression model.

        :param df: Input DataFrame containing records to predict.
        ...
        :return: Prediction result containing the model ID and predicted DataFrame.
        :rtype: RegressionPredictResult
